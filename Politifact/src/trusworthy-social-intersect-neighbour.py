...
BASE_PICKLE_PATH = "/home/shoaib/recommender-system/royal/journal-revision/New/Politifact/processed-files/TrustWorthy_Politifact_MAGrec_Constant.pickle"
...
def prepare_intersect_data(threshold):
    """
    Prepares data by intersecting the base social graph with the trustworthy neighbor graph.
    1. Loads the base social graph.
    2. Loads the trustworthy neighbor graph for the given threshold.
    3. Computes the intersection of connections for each user.
    4. Saves the new, intersected graph to a pickle file.
    """
    print(f"--- Starting Intersection Data Preparation for threshold: {threshold} ---")

    # Define paths for the two input graphs and the output graph
    neighbor_pickle_path = os.path.join(PROCESSED_FILE_DIR, f"D1_Trust_Neighbor_Update_{threshold}_threshold.pickle")
    output_pickle_path = os.path.join(PROCESSED_FILE_DIR, f"D1_Trust_Social_Intersect_Neighbor_Updated_{threshold}_threshold_Constant.pickle")
    ...
    if not os.path.exists(neighbor_pickle_path):
        ...
        print("Please run the 'run_trust_neighbour_space.py' script first for this threshold to generate the required file.")
        return None
    ...
    print("Computing the intersection of the two social graphs...")
    intersected_dict = defaultdict(set)
    # Iterate over users present in both graphs
    for key in social_adj_lists1.keys() & social_adj_lists2.keys():
        # Find the intersection of connections for the current user
        combined_set = social_adj_lists1[key] & social_adj_lists2[key]
        if combined_set:
            intersected_dict[key] = combined_set
    
    print(f"Intersection complete. New graph has {len(intersected_dict)} users.")
    ...
    data_to_save = (history_u_lists, history_ur_lists, history_v_lists, history_vr_lists,
                    train_u, train_v, train_r, test_u, test_v, test_r, intersected_dict, ratings_list)

    # Save the updated data to a new pickle file
    with open(output_pickle_path, 'wb') as file:
        pickle.dump(data_to_save, file)

    print(f"Intersected data saved to: {output_pickle_path}")
...
def train_and_recommend(args, final_pickle_path):
    """
    Handles model training and recommendation generation using the intersected social data.
    """
    ...
    for k in top_k_values:
        ...
        filename = f'{DATA_FILE_DIR}/Politifact_Trustworthy_Social_Intersect_Neighbor_{args.threshold}_threshold_Constant_top_{k}.csv'
...
def evaluate_model(threshold):
    """
    Calculates and saves MRR and MC metrics for the intersection model.
    """
    ...
    mrr_results = {}
    for k in top_k_values:
        recs_filename = f"{DATA_FILE_DIR}/Politifact_Trustworthy_Social_Intersect_Neighbor_{threshold}_threshold_Constant_top_{k}.csv"
    ...
    mrr_summary_path = os.path.join(RESULTS_DIR, f'trust_intersect_mrr_summary_{threshold}_threshold_Constant.csv')
    ...
    mc_summary_path = os.path.join(RESULTS_DIR, f'trust_intersect_mc_summary_{threshold}_threshold_Constant.csv')
...
def create_seed_files(threshold):
    ...
    for k in top_k_values:
        ...
        seed_dir = os.path.join(SEEDS_DIR, 'trustworthy_social_intersect_neighbor', f'threshold_{threshold}', f'top_{k}_Constant')
...
def main():
    parser = argparse.ArgumentParser(description='Social Recommendation (Intersection) with dynamic threshold.')
    ...
    final_pickle_path = os.path.join(PROCESSED_FILE_DIR, f"D1_Trust_Social_INTERSECT_Neighbor_Updated_{args.threshold}_threshold_Constant.pickle")

    if not args.skip_data_prep:
        final_pickle_path = prepare_intersect_data(args.threshold)
