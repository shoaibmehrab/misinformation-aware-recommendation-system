...
def prepare_trust_data(threshold, neighbor_size):
    """
    Performs all data preparation steps based on a dynamic threshold and neighbor size.
    """
    ...
    interaction_counts.fillna(0, inplace=True)
    
    # Identify trustworthy neighbors
    print(f"Identifying top {neighbor_size} trustworthy neighbors with reputation > {threshold}...")
    untrusted_users = interaction_counts[interaction_counts['reputation'] <= threshold]['user']
    ...
    for user in cosine_matrix.index:
        ...
        other_users_reputation = interaction_counts.loc[user_similarity.index, 'reputation']
        ...
        top_trustworthy_similar_users[user] = set(similar_trustworthy_users.sort_values(ascending=False).index[:neighbor_size].tolist())

    # Save untrusted users list
    # with open(os.path.join(PROCESSED_FILE_DIR, f'untrusted_users_ss{int(threshold*100)}.pickle'), 'wb') as f:
    #     pickle.dump(untrusted_users.tolist(), f)
    # print(f"Saved {len(untrusted_users)} untrusted users.")
    ...
    # Use threshold and neighbor_size in the final pickle filename
    final_pickle_path = os.path.join(PROCESSED_FILE_DIR, f"D1_Trust_Neighbor_Update_{threshold}_threshold_n{neighbor_size}.pickle")
...
def train_and_recommend(args, final_pickle_path):
    ...
    # Save top-k recommendations to CSV files with dynamic threshold and neighbor_size in filename
    top_k_values = [5, 10, 15]
    for k in top_k_values:
        recs_to_save = final_recs[(final_recs['rank'] <= k) & (final_recs['user'] != 0)]
        filename = f'{DATA_FILE_DIR}/Politifact_Trustworthy_Neighbor_{args.threshold}_threshold_n{args.neighbor_size}_top_{k}.csv'
...
def evaluate_model(threshold, neighbor_size):
    """
    Calculates and saves MRR and MC metrics based on the dynamic threshold and neighbor_size.
    """
    ...
    mrr_results = {}
    for k in top_k_values:
        recs_filename = f"{DATA_FILE_DIR}/Politifact_Trustworthy_Neighbor_{threshold}_threshold_n{neighbor_size}_top_{k}.csv"
    ...
    mrr_summary_path = os.path.join(RESULTS_DIR, f'trust_neighbour_mrr_summary_{threshold}_threshold_n{neighbor_size}.csv')
    ...
    mc_results = {}
    for k in top_k_values:
        # There was a typo with a double underscore here, correcting it.
        recs_filename = f"{DATA_FILE_DIR}/Politifact_Trustworthy_Neighbor_{threshold}_threshold_n{neighbor_size}_top_{k}.csv"
        recs_df = pd.read_csv(recs_filename)
    ...
    mc_summary_path = os.path.join(RESULTS_DIR, f'trust_neighbour_mc_summary_{threshold}_threshold_n{neighbor_size}.csv')
...
def create_seed_files(threshold, neighbor_size):
    """
    Creates seed files based on the generated recommendations for a given threshold and neighbor_size.
    """
    ...
    for k in top_k_values:
        # Define the directory for this k-value, including the neighbor_size
        seed_dir = os.path.join(SEEDS_DIR, 'trustworthy_neighbor', f'threshold_{threshold}_n{neighbor_size}', f'top_{k}')
...
def main():
    print("\n--- Starting Trustworthy Neighbour Recommendation Script ---")
    parser = argparse.ArgumentParser(description='Social Recommendation: GraphRec model with dynamic threshold and neighbor size.')
    ...
    parser.add_argument('--neighbor_size', type=int, default=10, help='Number of trustworthy neighbors to consider.')
    parser.add_argument('--skip_data_prep', action='store_true', help='Skip the data preparation step if files already exist.')
    args = parser.parse_args()

    print("\n--- Starting Trustworthy Neighbour Recommendation Script ---")
    final_pickle_path = os.path.join(PROCESSED_FILE_DIR, f"D1_Trust_Neighbor_Update_{args.threshold}_threshold_n{args.neighbor_size}.pickle")

    if not args.skip_data_prep:
        # Call prepare_trust_data but don't rely on its return value,
        # as it might be inconsistent.
        prepare_trust_data(args.threshold, args.neighbor_size)
    
    # After the preparation step (or if skipping), robustly check if the required file exists.
    if not os.path.exists(final_pickle_path):
        print(f"Error: Pickle file not found at {final_pickle_path}.")
        print("Please run the script without --skip_data_prep to generate the file first, or check for errors in the data preparation step.")
