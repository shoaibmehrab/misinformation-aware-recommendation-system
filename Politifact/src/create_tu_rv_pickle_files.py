"""
Script to create T_u-only and R_v-only pickle files for ablation studies.

This script:
1. Loads the original base pickle file
2. Computes T_u (user trustworthiness) from trainset/testset + news labels
...
TRAINSET_PATH = "/home/shoaib/recommender-system/royal/journal-revision/datasets/Politifact/trainset.csv"
TESTSET_PATH = "/home/shoaib/recommender-system/royal/journal-revision/datasets/Politifact/testset.csv"
POLITIFACT_PATH = "/home/shoaib/recommender-system/royal/journal-revision/datasets/Politifact/politifact.csv"

OUTPUT_TU_PATH = "/home/shoaib/recommender-system/royal/journal-revision/New/Politifact/processed-files/TrustWorthy_Politifact_MAGrec_Tu_only.pickle"
OUTPUT_RV_PATH = "/home/shoaib/recommender-system/royal/journal-revision/New/Politifact/processed-files/TrustWorthy_Politifact_MAGrec_Rv_only.pickle"
...
def compute_user_trustworthiness(trainset, testset, news_labels):
    """
    ...
    Args:
        trainset: DataFrame with columns [user, item, rating]
        testset: DataFrame with columns [user, item, rating]
        news_labels: DataFrame with columns [news_id_new, news_label]
    ...
    interactions_with_labels = all_interactions.merge(
        news_labels, 
        left_on='item', 
        right_on='news_id_new', 
        how='left'
    )
    
    # Calculate T_u for each user
    user_stats = interactions_with_labels.groupby('user').agg({
        'news_label': [
            ('total', 'count'),
...
def main():
    ...
    print(f"   Politifact: {len(politifact)} rows, {politifact['news_id_new'].nunique()} unique items")
    
    # Extract news labels
    news_labels = politifact[['news_id_new', 'news_label']].drop_duplicates()
    total_items = news_labels['news_id_new'].nunique()
    print(f"   Total unique items (N): {total_items}")
    print(f"   Labels: {news_labels['news_label'].value_counts().to_dict()}")
    
    # Compute T_u (user trustworthiness)
    print(f"\n3. Computing T_u (user trustworthiness)...")
    T_u = compute_user_trustworthiness(trainset, testset, news_labels)
