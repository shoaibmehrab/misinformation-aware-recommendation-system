...
def train_and_recommend(args, final_pickle_path):
    ...
    # Save top-k recommendations to CSV files with dynamic threshold in filename
    top_k_values = [5, 10, 15]
    for k in top_k_values:
        recs_to_save = final_recs[(final_recs['rank'] <= k) & (final_recs['user'] != 0)]
        filename = f'{DATA_FILE_DIR}/Politifact_Trustworthy_Neighbor_{args.threshold}_threshold_top_{k}.csv'
...
def evaluate_model(threshold):
    ...
    mrr_results = {}
    for k in top_k_values:
        recs_filename = f"{DATA_FILE_DIR}/Politifact_Trustworthy_Neighbor_{threshold}_threshold_top_{k}.csv"
    ...
    mrr_summary_path = os.path.join(RESULTS_DIR, f'trust_neighbour_mrr_summary_{threshold}_threshold.csv')
    ...
    mc_summary_path = os.path.join(RESULTS_DIR, f'trust_neighbour_mc_summary_{threshold}_threshold.csv')
...
def main():
    parser = argparse.ArgumentParser(description='Social Recommendation: GraphRec model with dynamic threshold.')
    ...
    # final_pickle_path = os.path.join(PROCESSED_FILE_DIR, f"D1_Trust_Neighbor_Update_{args.threshold}_threshold.pickle")
