...
def prepare_union_data(threshold):
    ...
    3. Computes the union of connections for each user.
    4. Saves the new, combined graph to a pickle file.
    """
    print(f"--- Starting Union Data Preparation for threshold: {threshold} ---")
    ...
    output_pickle_path = os.path.join(PROCESSED_FILE_DIR, f"D1_Trust_Social_UNION_Neighbor_Updated_{threshold}_threshold.pickle")
...
def train_and_recommend(args, final_pickle_path):
    ...
    for k in top_k_values:
        ...
        filename = f'{DATA_FILE_DIR}/Politifact_Trustworthy_Social_Union_Neighbor_{args.threshold}_threshold_top_{k}.csv'
...
def evaluate_model(threshold):
    ...
    mrr_summary_path = os.path.join(RESULTS_DIR, f'trust_union_mrr_summary_{threshold}_threshold.csv')
    ...
    mc_summary_path = os.path.join(RESULTS_DIR, f'trust_union_mc_summary_{threshold}_threshold.csv')
...
def create_seed_files(threshold):
    ...
    for k in top_k_values:
        ...
        seed_dir = os.path.join(SEEDS_DIR, 'trustworthy_social_union_neighbor', f'top_{k}')
...
def main():
    parser = argparse.ArgumentParser(description='Social Recommendation (Union) with dynamic threshold.')
    ...
    final_pickle_path = os.path.join(PROCESSED_FILE_DIR, f"D1_Trust_Social_UNION_Neighbor_Updated_{args.threshold}_threshold.pickle")
