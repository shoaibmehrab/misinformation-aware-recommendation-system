...
def prepare_intersect_data(threshold, neighbor_size):
    ...
    output_pickle_path = os.path.join(PROCESSED_FILE_DIR, f"D1_Trust_Social_Intersect_Neighbor_Updated_{threshold}_threshold_n{neighbor_size}.pickle")
...
def train_and_recommend(args, final_pickle_path):
    ...
    for k in top_k_values:
        ...
        filename = f'{DATA_FILE_DIR}/Politifact_Trustworthy_Social_Intersect_Neighbor_{args.threshold}_threshold_n{args.neighbor_size}_top_{k}.csv'
...
def evaluate_model(threshold, neighbor_size):
    ...
    mrr_results = {}
    for k in top_k_values:
        recs_filename = f"{DATA_FILE_DIR}/Politifact_Trustworthy_Social_Intersect_Neighbor_{threshold}_threshold_n{neighbor_size}_top_{k}.csv"
    ...
    mrr_summary_path = os.path.join(RESULTS_DIR, f'trust_intersect_mrr_summary_{threshold}_threshold_n{neighbor_size}.csv')
    ...
    mc_summary_path = os.path.join(RESULTS_DIR, f'trust_intersect_mc_summary_{threshold}_threshold_n{neighbor_size}.csv')
...
def create_seed_files(threshold, neighbor_size):
    ...
    for k in top_k_values:
        ...
        seed_dir = os.path.join(SEEDS_DIR, 'trustworthy_social_intersect_neighbor', f'threshold_{threshold}_n{neighbor_size}', f'top_{k}')
...
def main():
    ...
    final_pickle_path = os.path.join(PROCESSED_FILE_DIR, f"D1_Trust_Social_Intersect_Neighbor_Updated_{args.threshold}_threshold_n{args.neighbor_size}.pickle")
