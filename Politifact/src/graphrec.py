...
def train_and_recommend(args):
    """
    Handles model training and recommendation generation for a given Politifact pickle file.
    """
    ...
    # Correct logic for Politifact: item ID is index + 1
    ...
    # Correct cold start items for Politifact
    ...
    output_prefix = os.path.splitext(args.pickle_name)[0]
    for k in top_k_values:
        # Correctly exclude user 0 for Politifact
...
def mmc(dataframe, top_n):
    ...
    # Correct column name for Politifact
...
def evaluate_model(pickle_name):
    """
    Calculates and saves MRR and MC metrics for the Politifact baseline model.
    """
    print(f"\n--- Starting Evaluation for: {pickle_name} ---")
    
    # Correct test file and user exclusion for Politifact
    ...
    top_k_values = [5, 10, 15]
    output_prefix = os.path.splitext(pickle_name)[0]  # Base name for output files
    ...
    # MC Calculation
    # Correct key file and column names for Politifact
...
def create_seed_files(pickle_name):
    """
    Creates seed files based on the generated recommendations for the Politifact GraphRec model.
    """
    print(f"\n--- Creating Seed Files for: {pickle_name} ---")
    top_k_values = [5, 10, 15]
    output_prefix = os.path.splitext(pickle_name)[0]
    ...
    for k in top_k_values:
        ...
        seed_dir = os.path.join(SEEDS_DIR, 'graphrec', output_prefix, f'top_{k}')
...
def main():
    parser = argparse.ArgumentParser(description='Baseline GraphRec model for Politifact dataset.')
    parser.add_argument('--pickle_name', type=str, default='TrustWorthy_Politifact_MAGrec.pickle', help='Name of the input pickle file located in the processed-files directory.')
    ...
    # evaluate_model(args.pickle_name)
    create_seed_files(args.pickle_name)
