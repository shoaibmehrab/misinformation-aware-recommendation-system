...
def prepare_social_data(threshold):
    """
    Prepares data by filtering the social graph based on a pre-computed untrusted user list.
    1. Loads the base social graph.
    2. Loads the untrusted user list corresponding to the threshold.
    3. Removes untrusted users from the social adjacency lists.
    4. Saves the new, filtered data to a pickle file.
    """
    print(f"--- Starting Social Data Preparation for threshold: {threshold} ---")

    # Define paths based on the threshold
    untrusted_users_pickle_path = os.path.join(PROCESSED_FILE_DIR, f'untrusted_users_ss{int(threshold*100)}.pickle')
    output_pickle_path = os.path.join(PROCESSED_FILE_DIR, f"D1_Trust_Social_Update_{threshold}_threshold.pickle")

    # Check if the required untrusted users file exists
    if not os.path.exists(untrusted_users_pickle_path):
        print(f"Error: Untrusted users file not found at {untrusted_users_pickle_path}")
        print("Please run the 'run_trust_neighbour_space.py' script first to generate this file.")
        return None

    # Load base data with the original social graph
    with open(ORIGINAL_PICKLE_PATH, 'rb') as file:
        history_u_lists, history_ur_lists, history_v_lists, history_vr_lists, train_u, train_v, train_r, test_u, test_v, test_r, social_adj_lists, ratings_list = pickle.load(file)

    # Load the list of untrusted users
    with open(untrusted_users_pickle_path, 'rb') as file:
        untrusted_users = pickle.load(file)
    
    # The original notebook had `untrusted_users_ss60 = untrusted_users_ss60[1:]`.
    # This is kept for consistency, but be aware it might be removing a specific user (e.g., user 0).
    if untrusted_users:
        untrusted_users = untrusted_users[1:]
    print(f"Loaded {len(untrusted_users)} untrusted users to filter from the social graph.")

    # Create a new defaultdict to store the filtered social graph
    print("Filtering social graph by removing untrusted users...")
    filtered_social_adj_lists = defaultdict(set)
    untrusted_set = set(untrusted_users)

    for key, values in social_adj_lists.items():
        filtered_values = values - untrusted_set
        if filtered_values:  # Only add if the set is not empty after filtering
            filtered_social_adj_lists[key] = filtered_values
    ...
    data_to_save = (history_u_lists, history_ur_lists, history_v_lists, history_vr_lists,
                    train_u, train_v, train_r, test_u, test_v, test_r, filtered_social_adj_lists, ratings_list)

    # Save the updated data to a new pickle file
    with open(output_pickle_path, 'wb') as file:
        pickle.dump(data_to_save, file)

    print(f"Social data preparation complete. Filtered data saved to: {output_pickle_path}")
...
def train_and_recommend(args, final_pickle_path):
    """
    Handles model training and recommendation generation using the filtered social data.
    """
    ...
    for k in top_k_values:
        ...
        filename = f'{DATA_FILE_DIR}/Politifact_Trustworthy_Social_{args.threshold}_threshold_top_{k}.csv'
...
def evaluate_model(threshold):
    """
    Calculates and saves MRR and MC metrics based on the dynamic threshold.
    """
    ...
    mrr_results = {}
    for k in top_k_values:
        recs_filename = f"{DATA_FILE_DIR}/Politifact_Trustworthy_Social_{threshold}_threshold_top_{k}.csv"
    ...
    mrr_summary_path = os.path.join(RESULTS_DIR, f'trust_social_mrr_summary_{threshold}_threshold.csv')
    ...
    mc_summary_path = os.path.join(RESULTS_DIR, f'trust_social_mc_summary_{threshold}_threshold.csv')
...
def create_seed_files(threshold):
    ...
    test = pd.read_csv(f"{DATA_FILE_DIR}/politifact_test.csv").iloc[:, [1, 2, 3]]
    test.drop(index = 1028, inplace = True, errors ='ignore')
    key = pd.read_csv(KEY_FILE_PATH)
    key_news_id = key[["news_id_new", "news_label"]].drop_duplicates(subset=["news_id_new"], keep="last")
    key_news_id["fake_real_labels"] = key_news_id["news_label"].apply(lambda x: 0 if x == "fake" else 1)
    ...
    for k in top_k_values:
        ...
        seed_dir = os.path.join(SEEDS_DIR, 'trustworthy_social', f'threshold_{threshold}', f'top_{k}')
...
def main():
    parser = argparse.ArgumentParser(description='Social Recommendation (Social Space Filtering) with dynamic threshold.')
    parser.add_argument('--batch_size', type=int, default=128, metavar='N', help='input batch size for training')
    parser.add_argument('--embed_dim', type=int, default=64, metavar='N', help='embedding size')
    parser.add_argument('--lr', type=float, default=0.001, metavar='LR', help='learning rate')
    parser.add_argument('--test_batch_size', type=int, default=1000, metavar='N', help='input batch size for testing')
    parser.add_argument('--epochs', type=int, default=100, metavar='N', help='number of epochs to train')
    parser.add_argument('--threshold', type=float, default=0.6, help='Reputation threshold for filtering untrusted users.')
    parser.add_argument('--skip_data_prep', action='store_true', help='Skip the data preparation step if files already exist.')
    args = parser.parse_args()

    # final_pickle_path = os.path.join(PROCESSED_FILE_DIR, f"D1_Trust_Social_Update_{args.threshold}_threshold.pickle")

    # if not args.skip_data_prep:
    #     final_pickle_path = prepare_social_data(args.threshold)
