...
# Using the same consistent directory structure
...
def train_and_recommend(args):
    """
    Handles model training and recommendation generation for a given pickle file.
    """
    print(f"\n--- Starting Model Training for: {args.pickle_name} ---")
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"Using device: {device}")

    # The notebook uses a relative path, so we assume the pickle is in the same directory as the script
    # or we can use the processed files dir. Let's stick to the consistent path.
    ...
    # This is the top-k loop you requested
    top_k_values = [5, 10, 15]
    output_prefix = os.path.splitext(args.pickle_name)[0] # e.g., FH_GraphRec_Rating1
...
def evaluate_model(pickle_name):
    """
    Calculates and saves MRR and MC metrics for the baseline GraphRec model.
    """
...
def create_seed_files(pickle_name):
    """
    Creates seed files based on the generated recommendations for the baseline GraphRec model.
...
def main():
    parser = argparse.ArgumentParser(description='Baseline GraphRec model for FakeHealth dataset.')
    parser.add_argument('--pickle_name', type=str, default='FH_GraphRec_Rating1.pickle', help='Name of the input pickle file located in the processed-files directory.')
