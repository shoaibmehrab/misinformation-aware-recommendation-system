...
def prepare_intersect_data(threshold):
    ...
    with the trustworthy neighbor graph.
    """
    ...
    final_pickle_path = os.path.join(PROCESSED_FILE_DIR, f"FH_Trust_Social_Intersect_Neighbor_{threshold}_threshold.pickle")
...
def train_and_recommend(args, final_pickle_path):
    ...
    for k in top_k_values:
        ...
        filename = f'{DATA_FILE_DIR}/FH_Trustworthy_Social_Intersect_Neighbor_{args.threshold}_threshold_top_{k}.csv'
...
def evaluate_model(threshold):
    ...
    mrr_results = {}
    for k in top_k_values:
        recs_filename = f"{DATA_FILE_DIR}/FH_Trustworthy_Social_Intersect_Neighbor_{threshold}_threshold_top_{k}.csv"
    ...
    mrr_summary_path = os.path.join(RESULTS_DIR, f'fh_trust_intersect_mrr_summary_{threshold}_threshold.csv')
    ...
    mc_summary_path = os.path.join(RESULTS_DIR, f'fh_trust_intersect_mc_summary_{threshold}_threshold.csv')
...
def create_seed_files(threshold):
    ...
    for k in top_k_values:
        ...
        seed_dir = os.path.join(SEEDS_DIR, 'trustworthy_social_intersect_neighbour', f'threshold_{threshold}', f'top_{k}')
...
def main():
    ...
    final_pickle_path = os.path.join(PROCESSED_FILE_DIR, f"FH_Trust_Social_Intersect_Neighbor_{args.threshold}_threshold.pickle")
