...
BASE_PICKLE_PATH = os.path.join(PROCESSED_FILE_DIR, "TrustWorthy_FH_MAGrec_Constant.pickle")
...
def prepare_social_data(threshold):
    """
    Filters the social graph by removing untrusted users.
    """
    print(f"--- Starting Data Preparation for threshold: {threshold} ---")

    # Load the base dataset with the original social graph
    print(f"Loading base data from: {BASE_PICKLE_PATH}")
    ...
    # Load the list of untrusted users generated by the neighbor script
    untrusted_users_path = os.path.join(PROCESSED_FILE_DIR, f'untrusted_users_{threshold}_threshold.pickle')
    print(f"Loading untrusted users from: {untrusted_users_path}")
    try:
        with open(untrusted_users_path, 'rb') as file:
            untrusted_users = pickle.load(file)
    except FileNotFoundError:
        print(f"Error: Untrusted users file not found at {untrusted_users_path}")
        print(f"Please run the trustworthy-neighbour script first with --threshold {threshold} to generate it.")
        return None

    # Filter the social graph
    print("Filtering social graph to remove untrusted users...")
    filtered_social_graph = defaultdict(set)
    untrusted_users_set = set(untrusted_users)
    for key, values in social_adj_lists.items():
        # Remove untrusted users from the set of connections
        filtered_values = values - untrusted_users_set
        if filtered_values:  # Only add if the user still has connections
            filtered_social_graph[key] = filtered_values
    
    # Also, remove any untrusted users from being keys in the graph
    for user in untrusted_users:
        if user in filtered_social_graph:
            del filtered_social_graph[user]

    print(f"Original graph had {len(social_adj_lists)} users. Filtered graph has {len(filtered_social_graph)} users.")

    # Save the new dataset with the filtered social graph
    data_to_save = (history_u_lists, history_ur_lists, history_v_lists, history_vr_lists,
                    train_u, train_v, train_r, test_u, test_v, test_r, filtered_social_graph, ratings_list)

    final_pickle_path = os.path.join(PROCESSED_FILE_DIR, f"FH_Trustworthy_Social_{threshold}_threshold_Constant.pickle")
...
def train_and_recommend(args, final_pickle_path):
    ...
    for k in top_k_values:
        ...
        filename = f'{DATA_FILE_DIR}/FH_Trustworthy_Social_{args.threshold}_threshold_Constant_top_{k}.csv'
...
def evaluate_model(threshold):
    ...
    mrr_results = {}
    for k in top_k_values:
        recs_filename = f"{DATA_FILE_DIR}/FH_Trustworthy_Social_{threshold}_threshold_Constant_top_{k}.csv"
    ...
    mrr_summary_path = os.path.join(RESULTS_DIR, f'fh_trust_social_mrr_summary_{threshold}_threshold_Constant.csv')
    ...
    mc_summary_path = os.path.join(RESULTS_DIR, f'fh_trust_social_mc_summary_{threshold}_threshold_Constant.csv')
...
def create_seed_files(threshold):
    ...
    for k in top_k_values:
        ...
        seed_dir = os.path.join(SEEDS_DIR, 'trustworthy_social', f'threshold_{threshold}', f'top_{k}_Constant')
...
def main():
    parser = argparse.ArgumentParser(description='Trustworthy Social Space model for FakeHealth dataset.')
    parser.add_argument('--batch_size', type=int, default=128, metavar='N', help='input batch size for training')
    parser.add_argument('--embed_dim', type=int, default=64, metavar='N', help='embedding size')
    parser.add_argument('--lr', type=float, default=0.001, metavar='LR', help='learning rate')
    parser.add_argument('--test_batch_size', type=int, default=1000, metavar='N', help='input batch size for testing')
    parser.add_argument('--epochs', type=int, default=100, metavar='N', help='number of epochs to train')
    parser.add_argument('--threshold', type=float, default=0.6, help='Reputation threshold used to load the correct untrusted users file.')
    parser.add_argument('--skip_data_prep', action='store_true', help='Skip the data preparation step if files already exist.')
    args = parser.parse_args()

    final_pickle_path = os.path.join(PROCESSED_FILE_DIR, f"FH_Trustworthy_Social_{args.threshold}_threshold_Constant.pickle")
