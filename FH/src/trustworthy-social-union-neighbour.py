...
def prepare_union_data(threshold):
    """
    Creates a new social graph by taking the union of the original social graph
    and the trustworthy neighbor graph.
    """
    print(f"--- Starting Data Preparation for threshold: {threshold} ---")

    # Load the trustworthy neighbor graph
    neighbor_graph_path = os.path.join(PROCESSED_FILE_DIR, f"FH_Trustworthy_Neighbors_{threshold}_threshold.pickle")
    ...
    try:
        ...
        print(f"Please run the trustworthy-neighbour.py script first with --threshold {threshold} to generate it.")
        return None
    ...
    # Perform the union of the two social graphs
    print("Performing union of the two social graphs...")
    unioned_graph = social_adj_lists2.copy() # Start with a copy of the original graph
    for key, value in social_adj_lists1.items():
        unioned_graph[key].update(value) # Add new connections from the neighbor graph

    print(f"Resulting unioned graph has {len(unioned_graph)} users.")

    # Save the new dataset with the unioned social graph
    data_to_save = (history_u_lists, history_ur_lists, history_v_lists, history_vr_lists,
                    train_u, train_v, train_r, test_u, test_v, test_r, unioned_graph, ratings_list)

    final_pickle_path = os.path.join(PROCESSED_FILE_DIR, f"FH_Trust_Social_Union_Neighbor_{threshold}_threshold.pickle")
...
def train_and_recommend(args, final_pickle_path):
    ...
    for k in top_k_values:
        ...
        filename = f'{DATA_FILE_DIR}/FH_Trustworthy_Social_Union_Neighbor_{args.threshold}_threshold_top_{k}.csv'
...
def evaluate_model(threshold):
    ...
    mrr_results = {}
    for k in top_k_values:
        recs_filename = f"{DATA_FILE_DIR}/FH_Trustworthy_Social_Union_Neighbor_{threshold}_threshold_top_{k}.csv"
    ...
    mrr_summary_path = os.path.join(RESULTS_DIR, f'fh_trust_union_mrr_summary_{threshold}_threshold.csv')
    ...
    mc_summary_path = os.path.join(RESULTS_DIR, f'fh_trust_union_mc_summary_{threshold}_threshold.csv')
...
def create_seed_files(threshold):
    ...
    for k in top_k_values:
        ...
        seed_dir = os.path.join(SEEDS_DIR, 'trustworthy_social_union_neighbour', f'threshold_{threshold}', f'top_{k}')
...
def main():
    parser = argparse.ArgumentParser(description='Trustworthy Social Union Neighbor model for FakeHealth dataset.')
    ...
    final_pickle_path = os.path.join(PROCESSED_FILE_DIR, f"FH_Trust_Social_Union_Neighbor_{args.threshold}_threshold.pickle")

    if not args.skip_data_prep:
        final_pickle_path = prepare_union_data(args.threshold)
