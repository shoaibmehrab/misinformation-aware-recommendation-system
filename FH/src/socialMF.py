...
# --- 1. Configuration and File Paths for FakeHealth ---
...
# The social network file is expected here. Make sure it exists.
    ...
    def train(self, train_data, social_graph, epochs=100):
        ...
        for epoch in range(epochs):
            ...
            for _, row in tqdm(shuffled_data.iterrows(), total=len(shuffled_data), desc=f"Epoch {epoch+1}/{epochs}"):
                ...
                if u >= self.num_users or i >= self.num_items: continue
                ...
                if u in social_graph and social_graph[u]:
                    valid_friends = [f for f in social_graph[u] if f < self.num_users]
                    if valid_friends:
                        social_influence = np.mean(self.P[valid_friends], axis=0)
            ...
            print(f"Epoch {epoch+1}/{epochs}, RMSE Loss: {np.sqrt(total_loss / len(shuffled_data)):.4f}")
...
def generate_socialmf_recommendations(args):
    """
    Trains SocialMF and generates recommendations for FakeHealth. Returns True on success, False on failure.
    """
    try:
        ...
        if not os.path.exists(NETWORK_FILE_PATH):
            print(f"\n--- FATAL ERROR ---")
            print(f"Social network file not found at: '{NETWORK_FILE_PATH}'")
            print("Please ensure you have generated the 'network.txt' file for the FakeHealth dataset.")
            return False
    ...
    if not all_recs:
        print("\nWarning: No recommendations were generated. Output files will be empty.")
        return False
...
def evaluate_model(args):
    ...
    # FakeHealth-specific data cleaning
    if 5406 in test.index:
        test.drop(index=5406, inplace=True, errors='ignore')

    rla = topn.RecListAnalysis()
    rla.add_metric(topn.recip_rank)
    mrr_results = {}
    for k in args.top_k:
        recs_filename = f"{DATA_FILE_DIR}/FH_SocialMF_top_{k}.csv"
    ...
    mrr_summary_path = os.path.join(RESULTS_DIR, 'fh_socialmf_mrr_summary.csv')
    ...
    mc_summary_path = os.path.join(RESULTS_DIR, 'fh_socialmf_mc_summary.csv')
...
def main():
    parser = argparse.ArgumentParser(description='SocialMF Recommender for FakeHealth dataset.')
    ...
    success = generate_socialmf_recommendations(args)
    ...
    if success:
        ...
        print("\n--- Script finished with errors. Evaluation and seed creation were skipped. ---")
...
# 1.  Save the code above as `social_mf_fh.py` in your `/home/shoaib/recommender-system/royal/journal-revision/New/FH/src/` directory.
# 2.  Ensure the social network file `network.txt` exists in the corresponding `/data` directory.
# 3.  Open your terminal, navigate to the `src` directory, and run the script:

#     ```bash
#     python social_mf_fh.py --epochs 50
#     ```

# This will run the SocialMF model on your FakeHealth data and produce all the necessary output files for evaluation and spread analysis, keeping them separate from your other experiments.# filepath: /home/shoaib/recommender-system/royal/journal-revision/New/FH/src/social_mf_fh.py
