...
def prepare_trust_data(threshold, neighbor_size):
    ...
    print(f"Identifying top {neighbor_size} trustworthy neighbors (reputation > {threshold})...")
    ...
    for user in cosine_matrix.index:
        ...
        # --- MODIFIED LINE ---
        # Use the dynamic neighbor_size parameter instead of hardcoded 10
    ...
    # --- MODIFIED FILENAME ---
    # Add neighbor_size to the pickle filename to make it unique
    final_pickle_path = os.path.join(PROCESSED_FILE_DIR, f"FH_Trustworthy_Neighbors_{threshold}_threshold_n{neighbor_size}.pickle")
...
def train_and_recommend(args, final_pickle_path):
    ...
    for k in top_k_values:
        ...
        # --- MODIFIED FILENAME ---
        filename = f'{DATA_FILE_DIR}/FH_Trustworthy_Neighbor_{args.threshold}_threshold_n{args.neighbor_size}_top_{k}.csv'
...
def evaluate_model(threshold, neighbor_size):
    ...
    for k in top_k_values:
        # --- MODIFIED FILENAME ---
        recs_filename = f"{DATA_FILE_DIR}/FH_Trustworthy_Neighbor_{threshold}_threshold_n{neighbor_size}_top_{k}.csv"
    ...
    # --- MODIFIED FILENAME ---
    mrr_summary_path = os.path.join(RESULTS_DIR, f'fh_trust_neighbour_mrr_summary_{threshold}_threshold_n{neighbor_size}.csv')
    ...
    # --- MODIFIED FILENAME ---
    mc_summary_path = os.path.join(RESULTS_DIR, f'fh_trust_neighbour_mc_summary_{threshold}_threshold_n{neighbor_size}.csv')
...
def main():
    ...
    # --- MODIFIED FILENAME ---
    final_pickle_path = os.path.join(PROCESSED_FILE_DIR, f"FH_Trustworthy_Neighbors_{args.threshold}_threshold_n{args.neighbor_size}.pickle")

    if not args.skip_data_prep:
        # --- PASS NEW ARGUMENT ---
        final_pickle_path = prepare_trust_data(args.threshold, args.neighbor_size)
